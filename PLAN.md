...
Compare centered-square completion and Charge feedback on `/next/` against `/`, checking multi-layer awards, the neutral completed field, inward energy burst, Pulse scale, HUD update, twist overlap, focus, and responsive framing.

Once that checkpoint is accepted, continue slice 7 with the harvest result and presentation as its own reviewable increment. Do not create player-visible page swaps between title, puzzle, Pulse hub, Craft, or Board; do not add progression or remove the legacy controller.
